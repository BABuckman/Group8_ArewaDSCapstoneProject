...
vit_model.load_state_dict(torch.load("best_model_vit_b_16.pth", map_location=DEVICE))
vit_model.eval()

eff_model = EfficientNetB0(n_classes=2).to(DEVICE)
eff_model.load_state_dict(torch.load("best_model_efficientnet_b0.pth", map_location=DEVICE))
eff_model.eval()
...
# Reshape transform for ViT (required by Grad-CAM)
def vit_reshape_transform(tensor):
    # tensor shape: [batch_size, num_tokens, hidden_dim]
    # Remove class token (assume it's the first one)
    tensor = tensor[:, 1:, :]
...
def predict_with_gradcam(image, model_choice):
    ...
    # Select model and Grad-CAM target layer
    if model_choice == "ViT":
        model = vit_model
        ...
        model = eff_model
        target_layers = [model.features[6][0]]
...
def run_gradio_app():
    ...
    """) as demo:
        ...
        gr.Markdown("<img id='logo' src='ArewaDS_logo.PNG'>")
        gr.Markdown("<h2 class='centered' style='color:#e0b528;'>Deep Learning Fellow Cohort 2</h2>")
        gr.Markdown("""
<h3 class='centered' style='color:#e0b528;'>Group 8 - ArewaDS Capstone Project</h3>
<p class='centered'>
    <a href='https://github.com/BABuckman/Group8_ArewaDSCapstoneProject' target='_blank'>
        <img src='https://img.shields.io/badge/GitHub-View%20Project-181717?logo=github&logoColor=white&style=for-the-badge' alt='View on GitHub'>
    </a>
</p>
""")
